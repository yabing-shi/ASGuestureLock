...
@interface JYGestureNineButtonView ()
...
@implementation JYGestureNineButtonView

- (instancetype)initWithFrame:(CGRect)frame{
    self = [super initWithFrame:frame];
    if (self) {
        
        _isSetGesturePwd     = [JYHelper currentArchiveUserInfo].isSetGesPwd;
...
- (void)layoutSubviews{
    ...
    // 设置每个按钮的frame
    CGFloat w = dsize(80 * 3, 70 * 2, 60 * 2, 60 * 2);
    CGFloat h = dsize(80 * 3, 70 * 2, 60 * 2, 60 * 2);
    // 列（和行）的个数
    int columns = 3;
    // 计算水平方向和垂直方向的间距
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    if(/* DISABLES CODE */ (!_isSetGesturePwd)){//设置逻辑
        ...
        if (!_isLoginEntrance) {//修改逻辑
            ...
            //获取本地的密码
            NSString *localPassword = [NSString stringWithFormat:@"%@",[JYHelper currentArchiveUserInfo].gesPwd];
            DLog(@"%@",localPassword);
